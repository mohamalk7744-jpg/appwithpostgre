...
export const appRouter = router({
  ...
  subjects: router({
    ...
    update: protectedProcedure.input(z.object({ id: z.number(), name: z.string().optional(), description: z.string().optional(), numberOfDays: z.number().optional(), curriculum: z.string().optional() })).mutation(({ input }) => db.updateSubject(input.id, input)),
  ...
  quizzes: router({
    ...
    submit: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        // لاعتماد الدرجة فقط في المحاولة الأولى للاختبارات اليومية
  ...
  chat: router({
    ask: protectedProcedure.input(z.object({ subjectId: z.number(), question: z.string().min(1) })).mutation(async ({ ctx, input }) => {
      ...
      if (!subject || !subject.curriculum) return { answer: "لا يوجد منهج متاح لهذه المادة.", success: false };
      const answer = await askGemini(input.question, subject.curriculum, subject.name);
